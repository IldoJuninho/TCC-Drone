...
while True:
	...
	if webCamera.isOpened(): 
		...
		hsvp = cv2.inRange(hsv, lower_tp,upper_tp)
		hsvv = cv2.inRange(hsv, lower_tvf,upper_tvf)
		hsva = cv2.inRange(hsv, lower_blue,upper_blue)
		if state==2:
			detecta = classificadorBolinhaAzul.detectMultiScale(hsva)
		...
		if state==2:
			detecta = classificadorBolinhaAzul.detectMultiScale(hsvv)
			#detecta = classificadorCirculoInterno.detectMultiScale(cinza)
			# print(detecta)
			for(x, y, l, a) in detecta:
				#cv2.rectangle(frame, (x, y), (x + l, y + a), verde, 2)
				...
				#print(width+width*0.3,width-width*0.3,height+height*0.4,height-height*0.4)
		if state==1:
			detecta = classificadorAF.detectMultiScale(hsvp)
			#detecta = classificadorCirculoInterno.detectMultiScale(cinza)
			# print(detecta)
			for(x, y, l, a) in detecta:
				#cv2.rectangle(frame, (x, y), (x + l, y + a), verde, 2)
